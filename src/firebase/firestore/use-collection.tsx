...
export function useCollection<T = any>(
  ...
  useEffect(() => {
    if (!stableQuery || options?.disabled) {
      setData(null);
      setIsLoading(false);
      setError(null);
      return;
    ...
    const unsubscribe = onSnapshot(
      ...
      (error: FirestoreError) => {
        // Only handle permission errors with the custom logic
        if (error.code === 'permission-denied') {
          ...
          // Set the detailed error in the local state for the component to use
          setError(contextualError);
          
          // Emit the error globally for the Dev Overlay to catch
          errorEmitter.emit('permission-error', contextualError);
        } else {
          // For other types of errors (e.g., network), use the original error
          setError(error);
        }
        setData(null);
        setIsLoading(false);
